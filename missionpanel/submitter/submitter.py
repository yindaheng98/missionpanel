...
class Submitter:
    ...
    def match_mission(self, match_patterns: List[str]) -> Mission:
        matcher = self.session.query(Matcher).filter(or_(Matcher.pattern == pattern for pattern in match_patterns)).first()
        if matcher:
    ...
    def _add_tags(self, mission: Mission, tags_name: List[str]):
        exist_tags = self.session.query(Tag).filter(or_(Tag.name == tag_name for tag_name in tags_name)).all()
